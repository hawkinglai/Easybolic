...
from ops.manifold_layers import GeodesicLayer, MobiusLayer, LogZero, ExpZero, GyroplaneConvLayer
from manifolds import PoincareBall, Euclidean
import ops.nn as hypnn
import random
...
class Hype_Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        super(Hype_Model, self).__init__()
        self.stages = len(pre_blocks)
        self.class_num = class_num
        self.points = points

        self.manifold = PoincareBall(c=1,dim= 512)
        self.manifold2 = PoincareBall(c=1,dim= 256)
        ...
        self.act = get_activation(activation)
        # print(last_channel)
        self.proj = nn.Sequential(nn.Linear(last_channel, 512, bias=False),nn.BatchNorm1d(512) ,self.act)

        self.emb = MobiusLayer( 512  , 256 , self.manifold)
        self.classifier = MobiusLayer( 256 , self.class_num , self.manifold2)
        # print(last_channel)
    ...
    def forward(self, x, kn=10,  emb =False):
        xyz = x.permute(0, 2, 1)
        batch_size, _, _ = x.size()
        ...
        x = self.classifier(mu)
        return mu, child_mu, x


def Hype_pointMLP(num_classes=40, **kwargs) -> Hype_Model:
    return Hype_Model(points=1024, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0,
